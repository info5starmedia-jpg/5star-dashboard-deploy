...
export default function UsersPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/users").then((r) => r.json()).then((d) => { setUsers(d.users ?? []); setLoading(false); }).catch(() => { setError("Failed to load users."); setLoading(false); });
  }, []);
  async function toggleRole(email: string, currentRole: string) {
    const newRole = currentRole === "admin" ? "user" : "admin";
    setBusy(email);
    try {
      const res = await fetch("/api/admin/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, role: newRole }) });
  ...
  return (
    <main>
      ...
      {flash && <div style={{ marginBottom: 16, padding: "10px 16px", background: "#dcfce7", border: "1px solid #86efac", borderRadius: 8, color: "#166534", fontSize: 13 }}>{flash}</div>}
            ...
            {users.map((u, i) => (
              <tr key={u.email} style={{ borderTop: i > 0 ? "1px solid #f3f4f6" : "none", background: u.role === "admin" ? "#eff6ff" : "white" }}>
                ...
                <td style={{ padding: "10px 16px" }}>
                  <span style={{ display: "inline-block", padding: "2px 8px", borderRadius: 9999, fontSize: 11, fontWeight: 600, background: u.role === "admin" ? "#dbeafe" : "#f3f4f6", color: u.role === "admin" ? "#1d4ed8" : "#6b7280" }}>{u.role}</span>
                </td>
                ...
                <td style={{ padding: "10px 16px" }}>
                  <button disabled={busy === u.email} onClick={() => toggleRole(u.email, u.role)} style={{ padding: "4px 12px", borderRadius: 6, fontSize: 12, fontWeight: 600, cursor: busy === u.email ? "not-allowed" : "pointer", border: "1px solid", borderColor: u.role === "admin" ? "#fca5a5" : "#86efac", background: u.role === "admin" ? "#fee2e2" : "#dcfce7", color: u.role === "admin" ? "#dc2626" : "#16a34a", opacity: busy === u.email ? 0.6 : 1 }}>
