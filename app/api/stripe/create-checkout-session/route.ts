...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json().catch(() => ({})) as { planSlug?: string; quantity?: number };
    ...
    if (planSlug) {
      priceId = getPriceIdForPlan(planSlug);
      if (!priceId) {
        return NextResponse.json(
          { error: `No Stripe Price ID configured for plan: ${planSlug}` },
          { status: 400 }
        );
      }
    } else {
