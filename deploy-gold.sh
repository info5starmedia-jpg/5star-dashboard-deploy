...
if [ ! -d .git ]; then
  git init -q
  git config user.email "info.5starmedia@gmail.com"
  git config user.name  "Viking Essentials"
  git add -A                                   # .gitignore excludes .env, data/, node_modules
  git commit -qm "Gold production state before unified deploy" || echo "  (nothing to snapshot)"
fi
git remote remove origin 2>/dev/null || true
git remote add origin https://github.com/info5starmedia-jpg/5star-dashboard-deploy.git
git fetch --depth=1 origin "$BRANCH" || { echo "ABORT: git fetch failed (network?)"; exit 1; }
git checkout -f -B "$BRANCH" origin/"$BRANCH"  # writes branch, removes stale tracked files (old middleware.ts) — NO git clean
rm -f middleware.ts                            # belt-and-suspenders: ensure old middleware gone (proxy.ts is used now)
...
INV=$(python3 -c "import sqlite3; d=sqlite3.connect('file:data/app.db?mode=ro',uri=True); print(d.execute('SELECT COUNT(*) FROM InventoryItem').fetchone()[0], d.execute('SELECT COALESCE(SUM(quantity),0) FROM InventoryItem').fetchone()[0])" 2>/dev/null)
echo "  .env: OK | data/app.db: OK | proxy.ts: OK | middleware.ts: $( [ -f middleware.ts ] && echo STILL-PRESENT || echo removed )"
echo "  inventory (items totalQty): $INV"

# ---- STEP 4: build + restart (the one impactful step) ----
echo "[4/6] Building image and restarting container (brief downtime)..."
docker compose up -d --build || { echo "ABORT: docker build/up failed — old container may still be running. Review output above."; exit 1; }
...
docker compose ps
echo "--- recent logs ---"
docker compose logs --tail=30 app 2>/dev/null | grep -iE 'migration|listening|ready|started|error' | tail -20 || docker compose logs --tail=30 app
...
print("  InventoryItem has content:", 'content' in cols, "| subtitle:", 'subtitle' in cols)
...
PY
echo "==================================================================="
echo " DEPLOY COMPLETE. If HTTP is 200 and inventory looks right, you're live."
echo " Rollback if needed:  git checkout -f gold-production && docker compose up -d --build"
echo "==================================================================="
