...
//
// Env vars:
//   DISCORD_ALERT_WEBHOOK_URL / DISCORD_WEBHOOK_URL — Discord channel webhook (optional)
...
function getDiscordWebhookUrl() {
  return (process.env.DISCORD_ALERT_WEBHOOK_URL || process.env.DISCORD_WEBHOOK_URL || "").trim();
}

const DEFAULT_LOW_STOCK_THRESHOLD = 5;

function getLowStockThreshold() {
  const parsed = Number((process.env.LOW_STOCK_THRESHOLD || "").trim());
...
export async function notifyLowStock(items: LowStockItem[], source: string) {
  ...
  const lowStock = (items || []).filter(
    (item) => Number.isFinite(item.quantity) && item.quantity <= threshold
  );
